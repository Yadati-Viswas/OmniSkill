...
const StartCodingPage = () => {
    ...
    try {
        parsedConstraints = typeof problem.constraints === 'string' ? JSON.parse(problem.constraints) : problem.constraints;
        if (!Array.isArray(parsedConstraints)) parsedConstraints = [problem.constraints];
    } catch (e) {
                        ...
                        {parsedExamples && Array.isArray(parsedExamples) && parsedExamples.map((ex, idx) => (
                            <div key={idx} className={`p-4 rounded-lg ${darkMode ? "bg-gray-800" : "bg-gray-100"}`}>
                                ...
                                <div className="font-mono text-sm space-y-1">
                                    ...
                                    <p><span className="font-bold">Output:</span> {typeof ex.output === 'object' ? JSON.stringify(ex.output) : ex.output}</p>
