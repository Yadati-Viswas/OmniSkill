...
export default function ProblemsPage() {
    ...
    const SIZE = 15;
    const tags = ["Arrays", "Strings", "Dynamic Programming", "Trees", "Graphs", "Greedy", "Hash Table", "Sorting", "Recursion"];

    const fetchProblems = useCallback(async (currentPage, isReset = false) => {
        if (loading || (!hasMore && !isReset)) return;
        setLoading(true);
        try {
            const response = await getAllProblemsApi(currentPage, SIZE, searchQuery, selectedTag, sortBy);
            const newProblems = Array.isArray(response.data) ? response.data : (response.data.content || []);
